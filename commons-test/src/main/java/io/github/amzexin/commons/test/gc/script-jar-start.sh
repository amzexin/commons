...
# 指定OOM后要执行的shell脚本
execute_shell_on_oom="${current_directory}/ScriptOnOutOfMemoryError.sh"
...
#java_opts="${java_opts} -XX:OnOutOfMemoryError='sh ${execute_shell_on_oom}'" # 可以指定shell命令、shell脚本、其他脚本等，在OOM时执行一些额外的操作（实践证明，是先产生dump文件，再执行配置）
